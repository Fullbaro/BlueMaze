...
public class TorzsModosit extends Keret{
    ...
    void kiir(){
        ...
        if(nev.length()>10) hiba+="Az törzsvendég neve túl hosszú.<br>";
        
        flbr.sel=flbr.lekerdez("select nev from torzsvendegek where torolve like 0");
        for(String[] t: flbr.sel){
            if(t[0].toLowerCase().trim().equals(nev.toLowerCase().trim())) hiba+="Ilyen nevű törzsvendég már létezik";
        }
        
        if(hiba.length()!=0){
            new Uzen("Hiba",hiba).setVisible(true);
        }else{
           flbr.vegrehajt("update torzsvendegek set nev = '"+nev+"' where id = '"+lId.getText()+"'");
           new Uzen("Végrehajtva","A törzsvendég sikeresen módosítva.").setVisible(true);
           frissit();
           dispose();
        }
    }
    ...
    class ListaFigyel implements ListSelectionListener{
        public void valueChanged(ListSelectionEvent e){
            ...
            flbr.sel=flbr.lekerdez("select id from torzsvendegek where nev like '"+lista.getSelectedValue()+"'");
