...
public class BesorolasModosit extends Keret{
    ...
    public BesorolasModosit(){
        lCim.setText("Besorolás módosítása");
        ...
        pSor.add(lId); lId.setPreferredSize(new Dimension(20,10));
        pSor.add(lNev);
        pSor.add(tf);
    ...
    void kiir(){
        ...
        if(nev.length()>10) hiba+="A besorolás neve túl hosszú.<br>";
        
        flbr.sel=flbr.lekerdez("select megnevezes from besorolasok where torolve like 0");
        for(String[] t: flbr.sel){
            if(t[0].toLowerCase().trim().equals(nev.toLowerCase().trim())) hiba+="Ilyen nevű besorolás már létezik";
        }
        
        if(hiba.length()!=0){
            new Uzen("Hiba",hiba).setVisible(true);
        }else{
           flbr.vegrehajt("update besorolasok set megnevezes = '"+nev+"' where id = '"+lId.getText()+"'");
            frissit();
            dispose();
        }
    }
    ...
    void listaFeltolt(){
        flbr.sel=flbr.lekerdez("select megnevezes from besorolasok where torolve like 0 order by megnevezes");
        for (String[] t:flbr.sel)
    ...
    class ListaFigyel implements ListSelectionListener{
        public void valueChanged(ListSelectionEvent e){
            ...
            flbr.sel=flbr.lekerdez("select id from besorolasok where megnevezes like '"+lista.getSelectedValue()+"'");
